...
**反映が見えないときは、ユーザの操作より先にアプリを疑う。** 「まだ新しいバックアップが来ていない」
「保存先が別では」とユーザに聞き返さない。ユーザは「やった」と言っている以上やっている。
順番は次のとおり。

1. `~/Downloads` 以下を更新時刻で探し、`uscpa-backup-*.json` の最新世代を自分で特定する（Step 0 と同じ）
2. 見つかった最新世代が復元前と同一内容なら、**復元処理そのものが止まっている**可能性を先に調べる。
   復元の書き込み経路（`src/lib/backup/`）をプレビューで再現して確かめる
3. それでも原因がアプリ側に見つからないときに初めて、ユーザに画面の状態（スクショ）を頼む

2026-09-22 に、復元がIndexedDBへの事前スナップショット保存で止まったまま戻らないアプリの不具合を、
ユーザの操作ミスと取り違えて2回聞き返した（「だからやったって」「しつこい」「しらんがな」）。
その日に直したので、現在は**「復元しました」の表示は全件の書き込みが終わった後にしか出ない。**
ユーザがこの表示を確認していれば、再エクスポートでの確認は次回の検査に回してよい。

### Step 6. 集計と次アクションを出す

**この Step は依頼の主目的なので、省略しない。** ユーザの依頼は「確認よろしく」でも
「この先の学習を提案して」でも、求めているのは次に何をやるかであって、データの整合だけではない。

- `audit` で要対応が0件なら、Step 2 で止まらず**同じ返答で Step 6 まで出す**
- Step 5 の修正サイクルを閉じる返答（「反映されています」）にも、**そのまま集計と次アクションを付ける**。
  09-22 は整合確認だけで返して「時間かけてそれだけかよ」と言われ、そこから学習提案を出し直した
...
- **ただし検査で終わらせない。** 検査のあとには必ず集計と次アクションまで出す（Step 6）
- **反映が見えないときはアプリを先に疑う。** ユーザの操作を聞き返すのは最後（Step 5）
