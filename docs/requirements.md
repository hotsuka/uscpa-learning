...
#### 追加実装（v1.13）
- [x] 回答あり/なし2種類のPDF管理（pdfWithAnswers / pdfWithoutAnswers）
- [x] 回答あり/なし切り替えUI（ボタン + Vキーショートカット）
- [x] 教材一覧の検索機能
- [x] PDF文章検索機能（PDFViewer内蔵）
- [x] 水平/垂直レイアウト切り替え（localStorageで永続化）
- [x] キーボードショートカット
  - Ctrl+S / Cmd+S: メモ保存
  - `]` / `[`: ズームイン / ズームアウト
  - Space: タイマー開始/停止
...
#### 機能詳細
- **コンパクトタイマー**: 教材ページ内に埋め込まれたタイマー
- **問題数・正解数入力**: キーボードショートカット（Q/A）対応の入力フィールド
- **今日の合計勉強時間**: 当日の累計学習時間をリアルタイム表示
- **外部操作対応**: forwardRef + useImperativeHandleによる親コンポーネントからの操作

#### 受け入れ条件
- [x] 教材ページでタイマーを操作できる
- [x] 問題数・正解数をキーボードショートカットで操作できる
- [x] 今日の合計勉強時間が表示される

#### 追加実装（v1.13）
- [x] MiniTimer.tsx コンポーネントの実装
- [x] 問題数・正解数入力フィールド追加
- [x] 今日の合計勉強時間表示
- [x] forwardRef + useImperativeHandleによる外部操作
...
- [x] 未保存時の確認ダイアログ（戻るボタン時）
- [x] ブラウザ終了時のbeforeunload警告
- [x] 「未保存」バッジ表示
- [x] PC/モバイルの条件付きレンダリング（useMediaQueryフック）

#### 追加実装（v1.13）
- [x] ページめくり時のメモ自動保存（確認ダイアログ廃止）
- [x] useRefによる同期的な状態追跡（stale closure問題の回避）
- [x] localStorageからの最新メモ再読み込み
...
#### 追加実装（v1.13）
- [x] テーマ別統計機能（テーマごとの学習時間・問題数・正答率の詳細表示）
...
#### 受け入れ条件
- [x] 複数デバイス間でデータが同期される（Notion API経由、SyncProvider実装済み）
- [x] オフライン時の操作がオンライン復帰後に同期される（同期キュー + useSyncOnOnline実装済み）
- [x] タイマーのセッション情報が学習記録と紐付けられる（sessionId / source フィールド実装済み）
- [x] ページメモがノートDBに統合されて同期される（noteType: "page_memo" + syncMemoToNotion実装済み）
